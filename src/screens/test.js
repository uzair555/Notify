...
export class test extends Component {
    ...
    this.backHandler = BackHandler.addEventListener('hardwareBackPress', () => {
      Alert.alert(
        ...
        [
          ...
          { text: 'Logout', onPress: () => this.handleBack() },
        ],
        { cancelable: false },
      );
      return true;
  ...
  //   AsyncStorage.setItem('tm', this.state.message1).then(() => {});
  ...
  testNotification = () => {
    ...
    for (let i = 0; i < interval; i++) {
      PushNotification.localNotificationSchedule({
        ...
        date: new Date(Date.now() + Number(time) * (i + 1)), // in 60 secs
        allowWhileIdle: false, // (optional) set notification to work while on doze, default: false
        repeatType: 'day',
      });
    }
    this.props.navigation.navigate('TimeScreen', { Item: this.state.message1 });
  };
    ...
    return (
      <View style={styles.container}>
        <View style={{ flexDirection: 'row', width: 300 }}>
          ...
          <TouchableOpacity style={styles.btn1} onPress={this._Logout}>
            <Text
              style={{
                textAlign: 'center',
                fontSize: 18,
